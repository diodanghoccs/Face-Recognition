...
# Gradio UI
# ---------------------------------------------------------------------------
def format_result_md(name: str, r: dict) -> str:
    ...
    if "confidence" in r and r["confidence"] is not None:
        parts.append(f"- **Confidence:** {r['confidence']:.3f}")
    if r.get("similarity") is not None:
        parts.append(f"- **Similarity vs Reference:** {r['similarity']:.3f}")
    if "infer_ms" in r:
        parts.append(f"- **Time:** {r['infer_ms']:.0f} ms")
    return "\n".join(parts)
...
def build_ui(app: App):
    ...
    with gr.Blocks(title="Face Recognition - 5 Pipeline Demo", theme=gr.themes.Soft()) as demo:
        gr.Markdown("# Face Recognition - 5 Pipeline Demo\n"
                    "Upload **anh goc (Reference)** va chup **snapshot tu webcam/OBS**, "
                    "roi nhan **Run All** de chay 5 pipeline song song.")

        with gr.Row():
            ref_in  = gr.Image(label="Anh goc (Reference)", sources=["upload"], type="numpy", height=300)
            snap_in = gr.Image(label="Snapshot (Webcam / OBS)", sources=["webcam", "upload"], type="numpy", height=300)

        run_btn = gr.Button("Run All Pipelines", variant="primary")
        status_md = gr.Markdown(app.status_table())

        gr.Markdown("## Ket qua")
        with gr.Row():
            img_outs = [gr.Image(label=p.name, height=240, interactive=False) for p in app.pipes]
        with gr.Row():
            md_outs = [gr.Markdown() for _ in app.pipes]

        def _run(ref, snap):
            results = app.run_all(ref, snap)
            imgs = []
            mds = []
            for (name, r) in results:
                imgs.append(to_rgb(r["annotated"]) if r.get("annotated") is not None else None)
                mds.append(format_result_md(name, r))
            return imgs + mds

        run_btn.click(fn=_run, inputs=[ref_in, snap_in], outputs=img_outs + md_outs)
